...
| UX2.2b | "Drive it at 390, 768 and 1440." | **MET, 12 September 2026, on production** (was NOT MET and blocked: the Google browser key is referrer-restricted to the production host, so neither localhost nor a *.vercel.app preview can paint the map, proven on the preview of ee3d3408 with RefererNotAllowedMapError at 390 and 1440) | Driven on www.eventlinqs.com.au after the merge of #145: the map paints, the pin carries the venue name as title and text, sits inside the viewport, navy border, no key or referrer error, at 390, 768 and 1440. C:devEVIDENCEPRODUCTION-2026-09-12enue-pin-probe-production.txt and venue-pinenue-map-*.png |
...
## UX6, the mobile checkout layout (10 September 2026; CLOSED on the preview 12 September 2026, see "UX6 ON THE PREVIEW" below)
...
| UX6 | 1. Drive every checkout and ticket surface at 390, 768 and 1440 and capture each: event page, ticket select, checkout, payment, confirmation, ticket view, /tickets | **MET, 12 September 2026.** All seven surfaces at all three widths on the READY preview of a90c085a, the PAYMENT step included: 3 measured, 0 NOT EXERCISED, with a live Stripe frame (painted 25 to 27ms after the step appeared). 13 captures per width plus the chrome at 1024, 1100, 1280 and 1366 | C:\dev\EVIDENCE\UX6\preview-a90c085a\drive-run.txt, 390/paid-5-payment-step-viewport.png |
...
| UX6 | Completion law 5: the Stripe payment step | **MET, 12 September 2026.** Driven on the READY preview built from the pushed commit, which holds the TEST Stripe key: the step reached, painted, measured (0 clipped, total AUD 53.73 inside the viewport) at 390, 768 and 1440. It exposed a defect the layout work could not see (the scroll-anchoring drag, below), fixed in c4086acf and re-driven green on a90c085a | C:\dev\EVIDENCE\UX6\preview-a90c085a\, preview-0a195454\probe-390-m1.txt |
...
| UX6 | Completion law 7: committed, Australian English, no trailers, pushed | **MET, 12 September 2026.** e94840d6, c4086acf and a90c085a all on origin (origin at a90c085a, 0 ahead, 0 behind), each push through all fifteen gate steps | C:\dev\push-attempt.log lines 42727 to 46557 and 50094 to 53924 |
...
| UX6 tail | Pushed | **MET, 12 September 2026.** On origin with the rest of the branch at 0a195454 | push-attempt.log line 46556 |
...
| Pushed | **MET, 12 September 2026.** On origin at 0a195454 | push-attempt.log line 46556 |

## UX6 ON THE PREVIEW. 12 September 2026, commits c4086acf and a90c085a. The payment step driven for the first time, and what it showed.

| Item | Requirement | Verdict | Evidence |
|---|---|---|---|
| UX6 preview | The run brief: on a READY Vercel preview built from a pushed commit, driven at 390 on that preview, including the payment step | **MET.** dpl_7eXtiJX56iNs4erdwYuXqHzA2eRv READY, built from a90c085a, serving TEST; the drive at 390 (and 768, 1440) measured the payment step with a live Stripe frame; PASS, 0 faults | C:\dev\EVIDENCE\UX6\preview-a90c085a\drive-run.txt, ux6-report.json |
| UX6 preview | Fix every defect found | **MET.** THE DEFECT: after "Continue to payment" on a phone, Chrome's scroll anchoring followed Stripe's skeleton (+236px) and then its frame (+509px net) as they inserted above the fold, scrollY 145 to 381 to 890, leaving the buyer on the Pay button with every card field above the screen. Timed at 500ms intervals, not inferred. THE FIX (c4086acf): PaymentStep carries `overflow-anchor: none` on the whole step and starts at the top with focus on the Payment heading. After: scrollY 0 throughout the frame growing 2 to 412 to 747px, activeElement the heading, computed overflow-anchor none | preview-0a195454\probe-390-m1.txt (before), preview-a90c085a\probe-390.txt (after), src/app/checkout/[reservation_id]/checkout-form.tsx |
| UX6 preview | A capture artefact is not a defect, and is proven so | **MET.** The blank Stripe card and the mid-page skip link in the 390 full-page capture: under mobile emulation a full-page capture does not paint the cross-origin frame beyond the viewport, and the skip link sits at y=-59 unfocused with translateY(-150%). The scroll-0 viewport capture and the element capture of the frame show the fields painted | preview-0a195454\probe\390-mobilectx-top-viewport.png, 390-mobilectx-stripe-frame.png, probe-390-m1.txt (skip link state) |
| UX6 preview | Tests, canary raised in the same commit | **MET.** tests/component/checkout-payment-step.test.tsx, 4 tests: scroll to top on mount, focus on the heading, the overflow-anchor class on the step, the all-in total on the Pay button. Canary 379/4602 to 380/4606 | preview-0a195454\vitest-component.txt, canary.txt |
| UX6 preview | Proven to FAIL as well as pass | **MET.** With PaymentStep present and the fix removed the test fails exactly its three fix-holding cases and passes the fourth (3 failed, 1 passed); file restored, git diff clean. The drive's new assertion against the still-deployed pre-fix preview of 0a195454: "the Payment heading sits -762px from the top of a 844px viewport (scrollY 890)", exit 1; green on a90c085a | preview-a90c085a\component-test-red-2.txt, assertion-red-on-0a195454\drive-run.txt |
| UX6 preview | A registered blocking guard | **MET by the gate step, and the limit said plainly.** The assertion lives in the registered gate step checkout-viewport (PASS 217s on the push) and fires wherever a Stripe key exists, which locally is never; the component test holds the mechanism on every push. No separate source-level guard was written because it would only duplicate the test | scripts/ops/pre-push-gate.mjs, push-attempt.log line 53920 |
| UX6 preview | The proof reports Stripe's own markup rather than judging it | **MET.** axe critical aria-required-children on Stripe's payment-method selector inside iframe[name^="__privateStripeFrame"] at 390 and 1440: every node inside a Stripe or hCaptcha frame is printed, counted in the report (thirdPartyAxe 2) and the verdict line, and not judged; everything the platform authors is judged as before (40 scans, 0 faults). Not fixable in this repository | preview-a90c085a\drive-run.txt, ux6-report.json |
| UX6 preview | The proof enumerates a sellable event, never a guess | **MET (a90c085a).** The gate refused c4086acf because the picker chose kit-inspection-night-635605, whose organisation has no Stripe account, and the page rightly draws no selector. The picker now mirrors isOrganiserSellable, isExternallyTicketed and the tier sale window. TEST then held one sellable paid event with one place left behind 80 units of stale reservations; the product's own expire_stale_reservations() was run once on TEST (39 released) | push-attempt.log lines 46559 to 50092, preview-0a195454\test-sweep.txt, gate-checkout-viewport-local.txt |
| UX6 preview | Code typechecked, linted, no silent catches; full regression green; pushed | **MET.** tsc 0, eslint 0, all 111 guards, suite 380/4606, build, indexing, checkout-viewport, lighthouse (1684s on mains) on the push of a90c085a; origin at a90c085a | push-attempt.log lines 50094 to 53924 |
| UX6 preview | Roast gate | **PASSED.** 22 requirements, 22 met, 0 partial, 0 not met, 0 adversarial findings unresolved | docs/roast/ux6-preview-2026-09-12.md |
| UX6 preview | Housekeeping | **MET.** The UX6 body moved to CLOSE-OUT-DONE.md; one line left in CLOSE-OUT.md naming the commits, the date and the evidence | C:\dev\CLOSE-OUT.md, CLOSE-OUT-DONE.md |
...
| D1 | Backfill on PRODUCTION | **MET, 12 September 2026, 03:19, the one approved run.** BEFORE 0 rows; run 1 wrote 3 (EL-9HE57YNV x1, EL-UGPM3FQV x2, general admission 18.00); run 2 wrote 0 with 3 left alone; AFTER 3 rows, 1 slot; the slot of EL-9HE57YNV carries all three. Approval named on the command line, the shell approval given for that run only, the service_role key held in memory and never printed. Found in the same run: the child had no ORDER_ACCESS_SECRET, so the three rows are hashed with the empty key; answered by identityFingerprints so the engine still recognises those buyers, and the runner now refuses a write without the key | C:\dev\EVIDENCE\D1\2026-09-12\production-backfill.txt, runner-write-console.txt, refusal-a-no-approval.txt, refusal-b-no-shell-approval.txt, refusal-c-preflight.txt |
...
| Completion law 7: committed, Australian English, no trailers, pushed | **MET, 12 September 2026.** f053f7fc on origin with the branch at 0a195454; the production run and the fingerprint answer as 6f172972, pushed through the full gate | C:\dev\push-attempt.log |
...
## D1 ON PRODUCTION. 12 September 2026, commit 6f172972. The approved write, the hash it wrote without its key, and the render that is not mine.

| Item | Requirement | Verdict | Evidence |
|---|---|---|---|
| D1 production | The founder's approval: run the backfill against production once, only after the migration is on production, only what was genuinely recorded, report the count written and the count a second run writes (must be 0) | **MET.** Migration on production since 11 September (parity 126/126/0). Before 0; first run wrote 3; second run wrote 0; after 3. Only the two confirmed production orders' ticket lines, at the prices recorded on them; no demand rows, on purpose | C:\dev\EVIDENCE\D1\2026-09-12\production-backfill.txt |
| D1 production | Law 10: the founder's step as one command that proves itself | **MET.** scripts/ops/backfill-slot-ledger-production-once.mjs: refuses before it acts (three refusals drilled and captured), observes before and after, holds the credential in memory only, runs the real backfill twice, refuses success unless the ledger grew by exactly what run 1 reported and run 2 wrote 0 | runner-dry-run-console-2.txt, runner-write-console.txt, refusal-a/b/c |
| D1 production | The backfill refuses a production write by construction, and the one approval is explicit | **MET.** judgeBackfillTarget allows a production write only with --approved-by-founder named; assertNotProduction() then demands ALLOW_PRODUCTION_SUPABASE=1 in the shell and refuses a value parked in a file. Three tests | tests/unit/ledger/backfill.test.ts, refusal-c-preflight.txt |
| D1 production | The curve for the Afro-Fusion slot including EL-9HE57YNV, pulled | **MET.** Computed from production's rows with the panel's own buildCurve: days_out 31 (1 unit, 18.00) and 30 (2 units, 54.00 cumulative), totals 3 units / 54.00, demand all zero, close null | production-curve.txt |
| D1 production | ... and rendered, captured at 390, 768 and 1440, no overflow | **NOT DRIVEN HERE, and why.** The render lives on the organiser's dashboard on production (/dashboard/events/34a15c3a-c881-42f5-8ff2-d515232e966e); the event belongs to MKLStudios, whose one member is an outside organiser, and no admin surface renders the panel. Nothing on this machine can open it. The panel was driven on TEST on 10 September at all three widths on a denser real slot (15 of 15). Left open in CLOSE-OUT.md on this leg alone | production-curve.txt (the numbers the page would show); C:\dev\EVIDENCE\D1\ (the TEST render) |
| D1 production | Fix everything found on the way | **MET.** The three rows were hashed with the empty key (the child had no ORDER_ACCESS_SECRET). The ledger stays as written; identityFingerprints makes the engine's "already bought" and "money came back" rules ask with every shape an address can carry, keyed first; the runner refuses a write without the key from now on | src/lib/ledger/identity.ts, src/lib/fillrate/due.ts, waitlist.ts, read.ts, engine.ts |
| D1 production | Tests, canary raised in the same commit; proven red and green | **MET.** identity.test.ts (3), due (1), waitlist (1), backfill (3); canary 380/4606 to 381/4614. The two rule tests fail on the committed rules (rules-red.txt) and pass after | vitest-fingerprints-3.txt, rules-red.txt |
| D1 production | Regression green, pushed | **MET.** tsc 0, eslint 0, 111 guards, 381/4614; pushed through the full gate as 6f172972 | C:\dev\push-attempt.log |
...
| Re-attempted, session 75 at 15:34 | REFUSED IDENTICALLY at production-parity, step 9 of 15, same ten migrations; steps 1 to 8 PASS including all 110 guards (93s); environment half PASS (34 records, 0 faults); production read live at 15:34; origin still 27 behind after (re-fetched 15:38); no gate step touched, no bypass, no second push; nothing else started. Checked and written down: lifting the UX6 commit e94840d6 alone onto origin's tip, simulated read-only with git merge-tree, conflicts in three files (run-guards.mjs, test-count-canary.mjs, organisation-profile-form.tsx), so no side branch can carry it as the evidence cites it; the only path is the founder's command | C:\dev\push-attempt.log lines 13906 to 15449; BUILD-LOG.md, Session 75 |
| Re-attempted, session 76 at 15:42 | REFUSED IDENTICALLY at production-parity, step 9 of 15, same ten migrations on the same head 8e167886; steps 1 to 8 PASS including all 110 guards (92s); environment half PASS (34 records, 0 faults); production read live at 15:42; origin still 27 behind after (re-fetched 15:44); no gate step touched, no bypass, no second push; nothing else started; nothing new to check, both side-branch routes for the UX6 commit were already closed by sessions 74 and 75 | C:\dev\push-attempt.log lines 15451 to 16994; BUILD-LOG.md, Session 76 |
| Re-attempted, session 77 at 15:52 | REFUSED IDENTICALLY at production-parity, step 9 of 15, same ten migrations on head 8e167886; steps 1 to 8 PASS including all 110 guards; environment half PASS (34 records, 0 faults); origin 27 behind after | C:\dev\push-attempt.log lines 16996 to 18539; BUILD-LOG.md, Session 77 |
| Why the founder had not acted: had anything off this laptop told him? | ANSWERED: NO. The UX4.2 stall workflow is itself unpushed (HTTP 404 on the default branch), and run locally the stall judge read "0.1 hours ago, to ops/session-log" across a 44 hour silence, because every refused session's ledger push reset its clock (the last 30 activity records were all ops/session-log) | BUILD-LOG.md, Session 77 (a) and (b) |
| The stall judge defect, fixed under "fix every defect you find" | MET, commit 4d0fda21: BOOKKEEPING_REFS, pickLastPush and nextLinkPath in the lib, cursor paging past the bookkeeping in the collector, clause 6 of alert-routing (registered, blocking) drilled RED two ways and GREEN, 10 tests, canary 4574 to 4584, tsc and eslint clean, suite 378 files / 4584 tests / 0 failed / 0 skipped, build GREEN with every prebuild guard; Lighthouse not re-run because no src file changed | C:\dev\EVIDENCE\PUSH-2026-09-11\stall-clock-guard-red1-session77.txt, -red2-, -green-, regression-suite-build-session77.txt |
| The founder told through a channel that leaves the machine | MET, ONE alert: GitHub issue #149 through alert-dispatch, class stall, unmarked because it is not a drill. The email channel FAILED from here: RESEND_API_KEY is empty in .env.local (length 0, never printed) | https://github.com/eventlinqs/eventlinqs-app/issues/149; C:\dev\EVIDENCE\PUSH-2026-09-11\stall-alert-sent-session77.txt |
...
| Re-attempted, session 83 at 16:59, on head 4d0fda21 | REFUSED IDENTICALLY at production-parity, step 9 of 15, same ten migrations; parity read first (6 s, 10 pending) so the outcome was known before the gate; steps 1 to 8 PASS warm (typecheck 8s, lint 3s, all 110 guards 91s, types-drift 18s); environment half PASS (34 records, 0 faults); origin 28 behind after (re-fetched 17:02); issue #149 unanswered; no gate step touched, no bypass; nothing re-derived, nothing else started | C:\dev\push-attempt.log lines 27817 to 29361; BUILD-LOG.md, Session 83 |
| Re-attempted, session 84 at 17:06, on head 4d0fda21 | REFUSED IDENTICALLY at production-parity, step 9 of 15, same ten migrations; steps 1 to 8 PASS warm (typecheck 8s, lint 4s, all 110 guards 92s, types-drift 19s); environment half PASS (34 records, 0 faults); origin 28 behind after (re-fetched 17:09); issue #149 unanswered; the no-clean-subset fact re-enumerated from git (seven of the 28 commits add migrations, the oldest among them; origin's tip holds 116 migration files against 116 applied); no gate step touched, no bypass; nothing else started | C:\dev\push-attempt.log lines 29363 to 30907; BUILD-LOG.md, Session 84 |
| Re-attempted, session 85 at 17:15, on head 4d0fda21 | REFUSED IDENTICALLY at production-parity, step 9 of 15, same ten migrations; parity read first (6.8 s, 10 pending) so the outcome was known before the gate; steps 1 to 8 PASS warm (typecheck 8s, lint 4s, all guards 93s, types-drift 19s); environment half PASS (34 records, 0 faults); origin 28 behind after (re-fetched 17:19); issue #149 unanswered; no gate step touched, no bypass; nothing else started | C:\dev\push-attempt.log lines 30909 to 32453; BUILD-LOG.md, Session 85 |
| Did the one alert that left the laptop (issue #149) reach the founder? | NO, NOT AS ASSUMED. The issue is the founder's own account's activity (author eventlinqs, the acting login, the only collaborator), which GitHub emails only by the user's own choice ("Your own updates", docs fetched 11 September); sessions 77 to 84 were wrong to say it "reaches" him. The channel on record as reaching him is the failed-run email for runs his pushes trigger (close-out C2: six for one pull request) | BUILD-LOG.md, Session 85 |
| A stall alert that leaves the laptop, built where it can run and driven | MET. Workflow .github/workflows/stall-alert.yml on ops/session-log (from C:\dev\stall-alert.yml), state file STALL-STATE.json written by C:\dev\stall-state.mjs through C:\dev\push-build-log.ps1 on every publish, the platform's own dispatcher published unchanged. Driven: run 34574855048 (workflow_dispatch, actor eventlinqs) delivered on all three channels: Resend 200 accepted for hello@eventlinqs.com, issue #150 opened by app/github-actions, and the run FAILED on purpose so the failed-run email fires. Pacing proven: a second judge run on the same band left the file byte-identical. Found and fixed on the way: the first publish lost two paths to a bell character (run 34574674268 failed at the dispatcher step), and the third-channel step lacked always(). #149 closed with a pointer. Nothing in the code tree, the gate or production changed | C:\dev\EVIDENCE\PUSH-2026-09-11\stall-alert-run1-34574674268.log and stall-alert-run2-34574855048.log; C:\dev\STALL-STATE.json; https://github.com/eventlinqs/eventlinqs-app/issues/150; BUILD-LOG.md, Session 85 |
| Re-attempted, session 86 at 17:38, on head 4d0fda21 | REFUSED IDENTICALLY at production-parity, step 9 of 15, same ten migrations; parity read first (6 s, 10 pending) so the outcome was known before the gate; steps 1 to 8 PASS warm (typecheck 8s, lint 4s, all guards 92s, types-drift 20s); environment half PASS (34 records, 0 faults); origin 28 behind after (re-fetched 17:40); issue #150 unanswered (zero comments at 17:36); C16.2.1 re-read from CLOSE-OUT.md lines 459 to 464; band 7 already alerted so the publish fires nothing; no gate step touched, no bypass; nothing else started | C:\dev\push-attempt.log lines 32455 to 33999; BUILD-LOG.md, Session 86 |
| Re-attempted, session 87 at 17:46, on head 4d0fda21 | REFUSED IDENTICALLY at production-parity, step 9 of 15, same ten migrations; parity read first (6 s, 10 pending) so the outcome was known before the gate; steps 1 to 8 PASS warm (typecheck 8s, lint 3s, all guards 92s, types-drift 21s); environment half PASS (34 records, 0 faults); origin 28 behind after (re-fetched 17:48); issue #150 unanswered (zero comments at 17:45 and 17:48); band 7 already alerted so the publish fires nothing; no gate step touched, no bypass; nothing else started | C:\dev\push-attempt.log lines 34000 to 35545; BUILD-LOG.md, Session 87 |
...
## THE 500 ON PRODUCTION AFTER THE MERGE OF #145, FIXED FORWARD, AND THE GATE THAT LET IT THROUGH. Session 92, commit 2d85ce4a.

Not an item in CLOSE-OUT.md. The brief's MERGING rule: "if the deploy or the smoke fails, fix forward immediately and never remove anything from the platform". Adjudicated to the completion law.

| Requirement | Verdict | Evidence |
|---|---|---|
| The production smoke driven, 390 first, read only, after the merge | **MET** | post-deploy-smoke.mjs pinned to 011a9cd8 (live, homepage 200 anonymous and with el_city); the venue pin at 390, 768 and 1440 on production; the UX2 surfaces 30 of 30; axe 14 scans 0 violations; the route sweep, which found the defect | C:\dev\EVIDENCE\PRODUCTION-2026-09-12\ (smoke-production-console.txt and the files beside it) |
| The defect named from the evidence, not guessed | **MET** | /unsubscribe/recovery/zzzzzzzzzzzz answers 500; Vercel runtime error "the engine could not resolve that link: invalid input syntax for type uuid"; recovery_contacts.token is uuid, the sibling routes compare text | BUILD-LOG.md Session 92, entry 11 |
| Fixed at the cause, nothing removed | **MET** | isTokenShaped judges the shape; addressForToken answers null without asking for anything else; a real failure on a well-formed token still throws (the read-failure rule) | src/lib/fillrate/read.ts |
| Tests added, red before, green after; canary raised in the same commit | **MET** | tests/unit/fillrate/read.test.ts (6): RED 3 failed on the unfixed code; GREEN after. tests/unit/verify/upstash-stub-encoding.test.ts (5). Canary 382/4628 to 384/4639 | fix-500/read-test-red-before-fix.txt, tests-green-after-fix-2.txt |
| The local gate catches the class before a push, proven red then green | **MET** | New gate step route-sweep (after indexing, before Lighthouse) drives every route of the served build with --all-routes. RED on the old build: BLOCKED, 220 requests, the 500 named. GREEN on the fixed build: PASS, the route answers 404 | fix-500/gate-route-sweep-red-old-build-2.txt, gate-route-sweep-green-new-build.txt |
| The sweep's own faults on a local base fixed | **MET** | sitemap values rebased onto the base under test (twelve false dead links gone); an UNEXPLAINED public pattern is a defect; a stale allowlist entry is a note under --all-routes and a defect on the live site; the recovery route's note added | scripts/verify/production-route-sweep.mjs; fix-500/gate-route-sweep-red-old-build.txt (before) and -2.txt (after) |
| The gate's Upstash stub made faithful | **MET** | the stub base64-encodes string results when the client asks, as Upstash does; /api/health/redis under the gate server 503 with `"<(F"` before, 200 `PONG` after | scripts/verify/upstash-local-stub.mjs; BUILD-LOG.md entry 11 |
| Schema | n/a | No migration; the column is right, the code asked it wrongly | |
| Code typechecked, linted, no silent catches | **MET** | tsc 0; eslint 0 on every changed file; the build PASS through the gate's own build step (230s) | fix-500/tsc-2.txt, build-2.txt |
| Guard registered and blocking | n/a, and said so | No registered guard was added: the invariant is held by the suite (6 + 5 tests) and by the new gate step, which blocks every push; the drills cover registered guards and none changed | |
| Driven at 390, 768 and 1440 | **MET** for the surfaces this touched | The production smoke at the three widths (pin, UX2 surfaces, axe at 390 and 1440); the sweep is GET only by design | C:\dev\EVIDENCE\PRODUCTION-2026-09-12\ |
| Full regression green (the gate, now sixteen steps) and pushed | PENDING at the time of writing | The push of 2d85ce4a is running | push-attempt.log, the attempt headed head=2d85ce4a |
| Merged and production re-smoked with 0 defects | PENDING at the time of writing | | |

## THE PUSH REFUSED AT CHECKOUT-VIEWPORT: THE READ-FAILURE CLASS, FOURTH OCCURRENCE. Session 93, 12 September 2026, commit 6611d734.

Not an item in CLOSE-OUT.md. The brief's FIRST ACTION rule: "If the push is
refused at any other step, the refusal is your first work item: quote the exact
line that refused it, fix the cause properly, push again." Adjudicated to the
completion law.

| Requirement | Verdict | Evidence |
|---|---|---|
| ZERO ACTION: DEPLOY-STATE read first; UNKNOWN checked with gh before anything was treated as green | **MET** | origin tip ee3d3408: CI 34655051768 success, Lighthouse CI 34655051883 success; main 011a9cd8: CI, env locks and both post-deploy smoke runs success | BUILD-LOG.md Session 93, ZERO ACTION |
| The refusing line quoted | **MET** | "[ux6] FAIL: paid @ 768: the event page answered HTTP 404" and "[gate] BLOCKED at checkout-viewport (exit 1) after 267s. Nothing was pushed." | push-attempt.log, the attempt headed head=2d85ce4a; BUILD-LOG.md Session 93 |
| The cause named from the code, not the previous note | **MET** | The events layout's existence read (above the loading boundary, the real 404) discarded its error; the page's fetchEvent folded its error into null one layer down; the failing run's server log had been overwritten, so the class was reproduced on purpose in the proof | src/app/events/[slug]/layout.tsx at 2d85ce4a; BUILD-LOG.md Session 93, THE DIAGNOSIS |
| Fixed at the cause, one door, nothing removed | **MET** | src/lib/supabase/read-or-throw.ts; 20 decisive reads across 17 route files, 3 helpers and the archived-event path route through it; every `if (!x) notFound()` still stands on "no row" | git show 6611d734 --stat |
| Schema | n/a | No migration; the database was never wrong | |
| Code typechecked, linted, no silent catches | **MET** | tsc 0; eslint 0 on all 37 changed files; the preload's one catch around a file read speaks | BUILD-LOG.md Session 93, REGRESSION |
| Tests added, red before, green after; canary raised in the same commit | **MET** | 8 helper tests, 22 guard tests, the seo file rewritten (22), 7 fetch tests. The guard tests' tree sweep and the seo file's door assertions fail on the 2d85ce4a tree by construction (the door did not exist). Canary 384/4639 to 387/4691, MEASURED | scripts/guards/test-count-canary.mjs; .tmp/canary-readfail-2.txt |
| A registered blocking guard, proven to fail as well as pass | **MET** | read-failure-is-not-not-found registered (112 guards). RED on the 17 files as they stood at HEAD 2d85ce4a: 20 faults, exit 1. GREEN on the fixed tree. Three drills, one per fault, FAIL AS EXPECTED; 167 of 167 fire | C:\dev\EVIDENCE\READ-FAILURE-NOT-404\2026-09-12\guard-red-on-head-2d85ce4a.txt, run-guards-green.txt, guard-failure-drills.txt |
| Driven at 390, 768 and 1440 | **MET** | The blink proof on the production build: baseline 200; one dropped existence read retried 252 to 263ms later and 200; persistent failure four attempts then 500, never 404, no overflow; unknown slug 404 with the log saying so. Screenshots per width and leg | C:\dev\EVIDENCE\READ-FAILURE-NOT-404\2026-09-12\blink-proof\ |
| What the drive found that the tests could not, fixed in the item | **MET** | The retry never left the process: Next's render-time fetch deduplicator hands an identical GET the memo of the first failure. src/lib/supabase/undeduped-fetch.ts gives every Supabase request its own signal; the three client doors pass it; the second run of the proof shows the real retry | BUILD-LOG.md Session 93, THE DRIVEN PROOF; blink-proof/blink-injections.txt |
| The refusing step green on the fixed build before the push | **MET** | `npm run gate:push -- --only checkout-viewport`: PASS, 0 faults across 3 widths, 224s | .tmp/gate-checkout-viewport-readfail.txt |
| The local gate catches the class before a push | **MET** | The guard is in the registry the gate runs; the tree sweep is in the suite the gate runs; both red on the old shape | as above |
| Reported, not fixed, said plainly | **MET** | The checkout seat price resolver's discarded rpc error (a value with a default); the proxy's queue-gate read (lets the request through); two proxy passes per navigation | BUILD-LOG.md Session 93, REPORTED NOT FIXED |
| Committed, Australian English, no trailers, PUSHED | **MET for 6611d734** | 6611d734 committed 12:12, hook accepted; pushed 12:13 to 12:56, GREEN 16 of 16 in 2580s (BenchmarkIndex 2726, on mains), "ee3d3408..6611d734", origin at 6611d734; Vercel preview dpl_6gBfGFnvhLjNtdMVgVpkSyeteqUY READY | push-attempt.log, the attempt headed head=6611d734 |
| The pull request GitHub could build | **MET**, one merge commit | #151 read DIRTY: main is the squash of the branch's own 53 commits and the two new commits edit lines the squash also introduced. `git merge --no-ff -X ours origin/main` at b2564494 changes nothing (diff HEAD~1 HEAD empty; branch vs main is exactly the two commits, 44 files) | BUILD-LOG.md Session 93, THE PULL REQUEST GITHUB COULD NOT MERGE |
| The second push, b2564494 | REFUSED at checkout-viewport after 9s, the gate's own inventory: 77 of the paid event's 100 places in 38 reservations this drive had made and TEST never expires. Fixed in the drive (sweep before the pick, expire its own after), six pins, driven by hand (38 released, 0 faults, 3 expired after), committed 2a84de66 | BUILD-LOG.md Session 93, REFUSED AGAIN AT CHECKOUT-VIEWPORT; C:\dev\EVIDENCE\READ-FAILURE-NOT-404\2026-09-12\checkout-viewport-with-sweep.txt |
| The third push, 2a84de66 | REFUSED at Lighthouse after fifteen green steps: six floors missed by 0.01 to 0.08 with "Machine calibration: DEGRADED. BenchmarkIndex median 1844 ... 68% of the 2700 the floors were confirmed at"; the laptop on battery (PowerOnline False, 76 percent). The same tree cleared the step at 12:56 on mains and the commits since touch no page. No floor lowered, no step skipped. push-when-on-mains.ps1 started 14:24 (Law 10: the lead is IMPOSSIBLE for a machine, the rest is scripted) | push-attempt.log, the attempt headed head=2a84de66 |
| The fourth push, 2a84de66 on mains | **MET, 15:52** | push-when-on-mains.ps1 pushed at 15:04:38 once the lead was in (PowerOnline True 15:02:35); GREEN 16 of 16 in 2875s, BenchmarkIndex median 2657; origin at 2a84de66 | push-attempt.log, the attempt headed head=2a84de66 ahead=3 on mains |
| Watched to READY and green on every host | **MET, 16:26** | Vercel preview dpl_E7vzCLpACaGQd5qHtybd4pbkJ11b READY 15:55; CI 34676697830 success 15:57; Lighthouse CI 34676697824 success 16:26; #151 mergeStateStatus CLEAN, twelve checks SUCCESS or SKIPPED | gh pr view 151; BUILD-LOG.md Session 93, WATCHED GREEN |
| MERGING by the brief's rule | **MET, 16:26** | 57 messages swept (one prose mention of the file name CLAUDE.md, 0 dashes, 0 emoji); `gh pr merge 151 --squash`; main at d77aefce, 0 trailers | git log origin/main -1 |
| Production READY, main green, the automated smoke passed | **MET, 16:29 to 16:33** | GitHub deployment 6406775665 success; CI 34678148458 success; post-deploy smoke 34678267316 and 34678360541 success with production's CRON_SECRET | gh run list --commit d77aefce |
| The driven smoke on production, 390 first, read only | **MET, with two probe quirks named** | live as d77aefce; homepage 200 anonymous and with el_city; UX2 surfaces 30 of 30; axe 14 scans 0 violations (re-run with MSYS_NO_PATHCONV=1, now exported by the script); every declared route PASS; the venue pin PASS at 768 and 1440 and, after one cold-load miss at 390, PASS at all three on the re-run; sentinels NOT ASKED here and asked with the secret by the workflow | C:\dev\EVIDENCE\PRODUCTION-2026-09-12-b\ |

...
-- Called by the Stripe webhook once per charge.refunds.data[] entry.
-- One transaction: void this refund's tickets, return inventory,
-- reverse the ledger (mirroring the sale inverse), release/reduce the
-- reserve hold, set order + refund status. Idempotent: gated by the
-- refund row FOR UPDATE + status!='completed' latch flipped in the same
-- transaction, so webhook replays are a no-op. The webhook is the sole
-- money source of truth; this never runs on a Stripe failure.
...
BEGIN
  ...
  -- Balance portion: claw from available balance. May go negative if the
  -- reserve was already released/paid (existing negative_balance semantics).

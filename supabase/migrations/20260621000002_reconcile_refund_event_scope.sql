-- =====================================================================
-- 20260621000002_reconcile_refund_event_scope.sql
-- =====================================================================
-- Funds-holding model (docs/PAYMENTS-FUNDS-HOLDING.md Stage 6).
--
-- reconcile_refund must attribute every ledger delta to the order's EVENT, the
-- same way the sale (connect-ledger) and disbursement (disburse_transfer) now
-- do. Without event_id, a refund would reduce the org-wide balance but NOT the
-- event-scoped available the disbursement reads, so a refunded order could still
-- be disbursed in full. This CREATE OR REPLACE adds event_id (= the order's
-- event) to every ledger insert. The money/ticket/inventory logic is otherwise
-- unchanged from 20260531000001.
--
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN 'no_refund_row';   -- orphan path: caller voids tickets only, no ledger
  END IF;
  ...
  -- Organiser share clawed back for THIS refund = refund_amount - proportional fees.
  v_app_fee := round(
  ...
  -- Reserve portion: reserve_release (+R) undoes the original reserve_hold
  -- debit, refund_from_reserve (-R) claws it back; reduce/release the hold.
  -- event_id is attached so the event-scoped available reflects the refund.
  ...
  -- Balance portion: reduce the platform-held available for this event. Under
  -- the funds-holding model this normally just means we will transfer that much
  -- less to the organiser (pre-disbursement). If the event was already disbursed
  -- the app reverses the transfer and records the offsetting +adjustment, so the
  -- net event-available stays correct rather than going negative.
  ...
  -- Complete the refund row (idempotency latch for replays).

-- ============================================================
-- Door check-in scanner (launch-gate 5)
-- Run by founder: supabase db push --linked  (then: npm run db:types)
-- Design: docs/superpowers/specs/2026-05-31-door-checkin-scanner-design.md
-- ============================================================
-- Contents:
--   0. ticket_scans.result CHECK: add 'transferred' as a distinct reason
--   1. scan_ticket(p_ticket_code, p_secret, p_event_id) RPC
--      - SECURITY DEFINER, identity from auth.uid() (never a parameter)
--      - authorises: event-org owner, org member (owner/admin/manager),
--        or active platform admin
--      - admit-exactly-once via ONE atomic compare-and-set (valid -> scanned);
--        the row lock serialises concurrent scanners, exactly one admits
--      - rejects diagnose a distinct reason and append a ticket_scans audit row
--   2. GRANT EXECUTE to authenticated
-- tickets.ticket_code is already TEXT NOT NULL UNIQUE (unique btree index), so
-- the scan lookup is an instant index probe under a door queue. No new index.
-- AU English, no em-dashes. Online scanner only; offline is deferred.
-- ============================================================

-- ------------------------------------------------------------
-- 0. Audit enum: add 'transferred' so a transferred ticket is logged with its
--    own reason rather than collapsed into 'invalid'.
-- ------------------------------------------------------------
...
BEGIN
  ...
  -- Not found, or a secret mismatch. Do not behave as a secret oracle: both are
  -- reported as not_found. ticket_scans.ticket_id is NOT NULL, so a no-match
  -- cannot be audited (see the follow-up to make ticket_id nullable).

// Batch 11 - Lighthouse desktop pass on production build, 5 pages.
// Mobile pass already ran via inline command. Founder-authorised localhost run.
//
// Prereq: `npm run build` then `npm run start -- -p 3007` listening on
// http://localhost:3007.
import { execSync } from 'node:child_process'
import { existsSync, mkdirSync } from 'node:fs'

const BASE = 'http://localhost:3007'
const OUT = 'docs/redesign/batch-11-evidence/lighthouse'

if (!existsSync(OUT)) mkdirSync(OUT, { recursive: true })

const routes = [
  ['home',             '/'],
  ['events',           '/events'],
  ['community-african',  '/community/african'],
  ['city-sydney',      '/city/sydney'],
  ['event-detail',     '/events/diwali-festival-melbourne-festival-of-lights'],
]

console.log('=== Warm-up: curl every route twice ===')
for (let i = 0; i < 2; i++) {
  for (const [, path] of routes) {
    try {
      execSync(`curl -s -o /dev/null "${BASE}${path}"`, { stdio: 'ignore', timeout: 60_000 })
    } catch {}
  }
}

console.log('\n=== Desktop pass (5 routes) ===')
for (const [label, path] of routes) {
  const url = BASE + path
  console.log(`\n--- ${label} desktop ---`)
  const cmd = [
    'npx --yes lighthouse',
    `"${url}"`,
    '--quiet --output=json',
    `--output-path="${OUT}/${label}-desktop.json"`,
    '--chrome-flags="--headless=new --no-sandbox --disable-gpu"',
    '--preset=desktop',
    '--throttling-method=simulate',
  ].join(' ')
  try {
    execSync(cmd, { stdio: 'inherit', timeout: 240_000 })
  } catch (e) {
    console.log(`(non-fatal: ${e.message?.slice(0, 200)})`)
  }
}

console.log('\nDone.')

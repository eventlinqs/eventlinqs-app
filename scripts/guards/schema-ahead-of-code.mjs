...
if (absent.length > 0) {
  ...
  console.error(`${TAG}       step (CLAUDE.md, Verification and gates, Migrations). One command, in PowerShell from the repo:`)
  console.error(`${TAG}         npm run migrate:production`)
  console.error(`${TAG}       It lists what production lacks, asks for the production ref typed back, hands over the CLI's own`)
  console.error(`${TAG}       prompts, proves the result (this same manifest, asked of production) and rests the CLI on TEST.`)

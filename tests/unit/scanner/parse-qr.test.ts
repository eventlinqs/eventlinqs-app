...
import { parseScan, parseManual } from '@/lib/scanner/parse-qr'
...
describe('parseManual', () => {
  test('accepts a raw ticket code and a separate secret', () => {
    expect(parseManual('EL-7G4K-2PMQ', SECRET)).toEqual({ ticketCode: 'EL-7G4K-2PMQ', secret: SECRET })
  })

  test('upper-cases and trims a lower-case typed code', () => {
    expect(parseManual('  el-7g4k-2pmq  ', SECRET)).toEqual({ ticketCode: 'EL-7G4K-2PMQ', secret: SECRET })
  })

  test('accepts a full bearer URL pasted into the manual code field', () => {
  ...
  test('returns null when the secret is missing and the code is not a URL', () => {

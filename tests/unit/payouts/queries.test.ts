...
// Pricing-rules is the shared source for reserve % and cadence days. Mock it so
// the terms test is deterministic and does not touch the DB. The real functions
// are exercised by the payments suite.
vi.mock('@/lib/payments/pricing-rules', () => ({
  getReservePercentage: vi.fn(async () => 20),
...
interface MockState {
  ...
  rpcCalls: { fn: string; args: AnyRecord }[]
...
function makeState(overrides: Partial<MockState> = {}): MockState {
  return {
    ...
    ledger: [],
    ledgerCount: null,
    org: { stripe_account_country: 'AU', payout_tier: 'tier_1', payout_schedule: 'post_event_only' },
    availableBalance: 0,
    rpcCalls: [],
    capturedQueries: [],
    ...overrides,
 ...
 * tests can assert against the captured filter set. Tolerant: any chained
 * call returns the same builder, terminal `await` resolves a configured payload.
 */
function makeMockClient(state: MockState): SupabaseClient {
  ...
  const rpc = vi.fn((fn: string, args: AnyRecord) => {
    state.rpcCalls.push({ fn, args })
    if (fn === 'organiser_available_balance') {
      return Promise.resolve({ data: state.availableBalance, error: null })
    }
...
describe('getOrganiserPayouts', () => {
  ...
  test('filters currency case-insensitively (ilike, no case mangling)', async () => {
    const state = makeState({ payouts: [], payoutsCount: 0 })
    const client = makeMockClient(state)

    await getOrganiserPayouts('org-1', { currency: 'AUD' }, client)
    const q = state.capturedQueries.find((q) => q.table === 'payouts')
    expect(q?.filters['ilike:currency']).toBe('AUD')
  })
...
describe('getOrganiserPayoutSummary', () => {
  test('aggregates pending, paid this month, on hold, lifetime totals', async () => {
    const state = makeState({
      ...
      payoutHolds: [{ amount_cents: 2000, currency: 'AUD' }],
    })
    const client = makeMockClient(state)
  ...
  test('available balance comes from organiser_available_balance RPC with AUD (matches admin)', async () => {
    const state = makeState({ availableBalance: 73210 })
    const client = makeMockClient(state)
    const summary = await getOrganiserPayoutSummary('org-1', client)

    expect(summary.availableCents).toBe(73210)
    expect(summary.currency).toBe('AUD')
    const rpc = state.rpcCalls.find((c) => c.fn === 'organiser_available_balance')
    expect(rpc?.args).toEqual({ p_organisation_id: 'org-1', p_currency: 'AUD' })
  })

  test('aggregates holds in AUD only (matches admin cockpit hold filter)', async () => {
    const state = makeState({ payoutHolds: [{ amount_cents: 999, currency: 'AUD' }] })
    const client = makeMockClient(state)
    await getOrganiserPayoutSummary('org-1', client)
  ...
  test('available defaults to 0 when RPC returns null', async () => {
    const state = makeState({ availableBalance: 0 })
    const client = makeMockClient(state)
    const summary = await getOrganiserPayoutSummary('org-1', client)
...
describe('getOrganiserPayoutTerms', () => {
  test('maps tier code to label and pulls reserve/cadence from pricing rules', async () => {
    const state = makeState({
      org: {
        payout_tier: 'tier_1',
    ...
    const client = makeMockClient(state)
    const terms = await getOrganiserPayoutTerms('org-1', client)

    expect(terms.tier).toBe('tier_1')
    expect(terms.tierLabel).toBe('Standard')
    expect(terms.reservePercent).toBe(20)
    expect(terms.cadenceDays).toBe(3)
    expect(terms.onDemandEligible).toBe(false)
  })

  test('flags on-demand eligibility for scheduled_plus_on_demand', async () => {
    const state = makeState({
      org: {
        payout_tier: 'tier_2',
        payout_schedule: 'scheduled_plus_on_demand',
        stripe_account_country: 'AU',
      },
    })
    const client = makeMockClient(state)
    const terms = await getOrganiserPayoutTerms('org-1', client)
    expect(terms.tierLabel).toBe('Verified')
    expect(terms.onDemandEligible).toBe(true)
  })

  test('defaults to tier_1/Standard when org has no tier set', async () => {
    const state = makeState({ org: null })
    const client = makeMockClient(state)
    const terms = await getOrganiserPayoutTerms('org-1', client)
    expect(terms.tier).toBe('tier_1')
    expect(terms.tierLabel).toBe('Standard')

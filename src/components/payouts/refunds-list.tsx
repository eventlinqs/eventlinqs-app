  ...
  return (
    ...
    >
      <header className="flex items-start justify-between gap-3">
        ...
        <span className="flex h-9 w-9 items-center justify-center rounded-full bg-error/10 text-error" aria-hidden="true">
          <Undo2 className="h-4 w-4" />
        </span>
      </header>

      {page.rows.length === 0 ? (
        <p className="mt-6 rounded-lg bg-ink-100/60 px-4 py-6 text-center text-sm text-ink-600">
        ...
        <ul className="mt-5 divide-y divide-ink-100">
          {page.rows.map((row) => (
            <li key={row.id} className="flex flex-wrap items-center justify-between gap-3 py-3">
              ...
              <span
                className={`font-display text-base font-bold tabular-nums ${row.delta_cents < 0 ? 'text-error' : 'text-success'}`}
              >
                {row.delta_cents < 0 ? '-' : '+'}

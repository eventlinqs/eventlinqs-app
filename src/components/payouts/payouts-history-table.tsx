    ...
    >
      <header className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select
            ...
            className="min-h-[44px] rounded-lg border border-ink-200 bg-white px-3 py-2 text-sm text-ink-900 focus:border-gold-500 focus:outline-none focus:ring-2 focus:ring-gold-400"
          >
      ...
      <div className="mt-4 overflow-x-auto">
        <table className="min-w-full divide-y divide-ink-100 text-sm">
          ...
          <tbody className="divide-y divide-ink-100">
            {page.rows.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-3 py-10 text-center text-sm text-ink-400">
      ...
      <footer className="mt-4 flex items-center justify-between gap-3 text-xs text-ink-600">
        ...
        <div className="flex items-center gap-2">
          {isPending && (
            <Loader2 className="h-3.5 w-3.5 animate-spin text-ink-400" aria-hidden="true" />
          )}
          <button
            type="button"
            disabled={!canPrev || isPending}
            onClick={() => fetchPage({ status, offset: Math.max(0, page.offset - page.limit) })}
            className="min-h-[36px] rounded-lg border border-ink-200 bg-white px-3 py-1.5 text-xs font-semibold text-ink-900 disabled:cursor-not-allowed disabled:opacity-40 hover:border-ink-400"
          >
            Previous
          </button>
          <button
  ...
  return (
    <tr className="text-ink-900">
      <td className="px-3 py-3 whitespace-nowrap">
        ...
        {row.failure_reason && (
          <p className="text-xs text-error">{row.failure_reason}</p>
        )}
      </td>
      ...
      <td className="px-3 py-3 font-mono text-xs text-ink-400">{row.stripe_payout_id}</td>
    </tr>
  )
}

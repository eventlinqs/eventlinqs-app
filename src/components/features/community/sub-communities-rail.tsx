...
import { SubCommunityTileImage } from '@/components/media/SubCommunityTileImage'
import type { SubCommunity } from '@/lib/communities/data'
...
interface Props {
  ...
  /**
   * Map of sub-community slug → Pexels landscape URL (or null if Pexels
   * returned nothing or PEXELS_API_KEY is missing). Sub-communities with
...
/**
 * SubCommunitiesRail - 6 photographic sub-community tiles.
 *
 ...
 * with a soft shadow. Tile click routes to /events?community={communitySlug}&sub={subSlug}.
 */
export function SubCommunitiesRail({ communitySlug, communityName, subCommunities, images }: Props) {
  if (subCommunities.length === 0) return null
  return (
    <ContentSection surface="alt" width="default" topBorder reveal>
      <div className="mb-8 flex items-end justify-between gap-4">
        <div>
          <p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--brand-accent-strong)]">
            Inside {communityName}
          </p>
      ...
      >
        {subCommunities.map((sc) => {
          ...
          return (
            <li key={sc.slug}>
              <Link
                href={`/events?community=${communitySlug}&sub_community=${sc.slug}`}
              ...
              >
                <div className="relative aspect-[4/3] w-full overflow-hidden bg-[var(--color-navy-950)]">
                  {img ? (
                    <SubCommunityTileImage
                      src={img}
                      alt={`${sc.label} - ${communityName}`}

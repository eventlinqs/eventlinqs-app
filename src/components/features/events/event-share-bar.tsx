...
export function EventShareBar({ eventTitle, eventDate, eventUrl, variant = 'light' }: Props) {
  ...
  const links = [
    ...
    },
    {
      ...
      bg: variant === 'dark' ? 'bg-white/10 hover:bg-white/20' : 'bg-[var(--surface-1)] hover:bg-[var(--surface-2)]',
    ...
    <div className="flex flex-wrap items-center gap-2">
      {links.map(l => (
        <a
          ...
          className={`inline-flex h-10 items-center gap-2 rounded-full px-4 text-sm font-semibold text-white transition-all hover:-translate-y-0.5 ${l.bg}`}
        >
          {l.icon}

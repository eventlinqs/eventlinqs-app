...
/**
 * HeroCarouselClient - clean, short, single-feature hero (Ticketmaster /
 ...
 * rolling progress bar, no busy motion. Height is roughly half the old
 * full-bleed treatment and the title is dialled back so the hero reads as
 * a confident banner, not a takeover.
 *
...
export function HeroCarouselClient({ slides }: Props) {
  ...
  // Announce slide changes politely (no auto-advance, only manual nav).
  useEffect(() => {
  ...
  return (
    ...
    >
        ...
        return (
          ...
          >
            ...
            <div
              ...
              style={{
                background:
                  'linear-gradient(to top, rgba(10,22,40,0.82) 0%, rgba(10,22,40,0.45) 45%, rgba(10,22,40,0.10) 80%, rgba(10,22,40,0.00) 100%)',
              }}
            />
            {/* Content - bottom-left, dialled back vs the old takeover. */}
      ...
      {/* Arrows (desktop). Always visible on hover/focus; 44px target. */}
      <button
        type="button"
        onClick={prev}
      ...
      {/* Subtle dots (bottom-centre), 24px hit zone each. */}
      <div
        role="tablist"

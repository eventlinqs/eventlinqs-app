...
export default async function OrderDetailPage({ params }: Props) {
  ...
  const statusInfo = STATUS_LABELS[fullOrder.status] ?? { label: fullOrder.status, className: 'bg-gray-100 text-gray-600' }
  ...
  return (
    <div>
      <div className="mb-6 flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Order {fullOrder.order_number}</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-6">
            <h2 className="text-base font-semibold text-gray-900 mb-4">Buyer</h2>
            <div className="text-sm text-gray-700 space-y-1">
              <p><span className="text-gray-400">Name:</span> {buyerName || '—'}</p>
              <p><span className="text-gray-400">Email:</span> {buyerEmail}</p>
              {!fullOrder.user_id && (
                <p className="text-xs text-gray-400 mt-2">Guest checkout</p>
              )}
            </div>
          </div>
              ...
              {ticketItems.map(item => (
                <div key={item.id} className="py-3">
                  <div className="flex justify-between text-sm">
                    <div>
                      <p className="font-medium text-gray-900">{item.item_name} ×{item.quantity}</p>
                    ...
                    <span className="text-gray-900">{formatCents(item.total_cents, fullOrder.currency)}</span>
                  </div>
                </div>
              ))}
              {addonItems.map(item => (
                <div key={item.id} className="py-3">
                  <div className="flex justify-between text-sm">
                    <p className="text-gray-700">{item.item_name} ×{item.quantity}</p>
                    <span className="text-gray-900">{formatCents(item.total_cents, fullOrder.currency)}</span>
                  </div>
                </div>
              ))}
              ...
              <div className="text-sm text-gray-700 space-y-2">
                ...
                {payment.receipt_url && (
                  <div className="flex justify-between">
                    ...
                    <a href={payment.receipt_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline text-xs">
                      View receipt
                    </a>
                  </div>
                )}
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-5">
            <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Breakdown</h3>
            <div className="space-y-2 text-sm">
              ...
              <div className="border-t border-gray-100 pt-2 flex justify-between font-bold text-green-700">
                <span>Your Revenue</span>
                <span>{formatCents(organiserRevenue, fullOrder.currency)}</span>
              </div>
            </div>
            <p className="mt-3 text-xs text-gray-400">Payouts processed after the event</p>
          </div>

          {/* Order metadata */}
          <div className="rounded-xl border border-gray-200 bg-white p-5 text-xs text-gray-500 space-y-1.5">
            <div className="flex justify-between">
              <span>Placed</span>

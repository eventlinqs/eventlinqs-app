  ...
  if (!org) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <h2 className="text-xl font-semibold text-gray-900">No Organisation Yet</h2>
    ...
    <div className="max-w-3xl">
      <div className="mb-8 flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">eventlinqs.com/{org.slug}</p>
        </div>
        <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-medium capitalize ${statusColour[org.status] ?? 'bg-gray-100 text-gray-500'}`}>
          {org.status}
        </span>
      </div>
      ...
      <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          ...
          <p className="mt-1 text-3xl font-bold text-gray-900">{eventCount ?? 0}</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          <p className="text-xs text-gray-500 uppercase tracking-wide">Team Members</p>
          <p className="mt-1 text-3xl font-bold text-gray-900">{memberCount ?? 0}</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-5">
      ...
      <div className="rounded-xl border border-gray-200 bg-white divide-y divide-gray-100">
        {org.description && (
          <div className="px-6 py-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-500 mb-1">About</p>
            <p className="text-sm text-gray-700">{org.description}</p>
          </div>
        )}
        {org.website && (
          <div className="px-6 py-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-500 mb-1">Website</p>
        ...
        {org.email && (
          <div className="px-6 py-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-500 mb-1">Contact</p>

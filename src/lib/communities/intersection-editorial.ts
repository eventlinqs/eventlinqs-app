...
const INTERSECTIONS: Record<string, IntersectionEntry> = {
  ...
  'middle-eastern/perth': {
    ...
    editorial:
      "Perth's Middle Eastern community is established and family-led. Cannington holds the Lebanese families, Willetton holds the Persian community, Ferndale holds the Iraqi and Syrian families, Nollamara holds the Turkish community. Lebanese Mahrajan in Perth every May at Whiteman Park pulls six thousand, Persian Norouz at Forrest Place every March pulls three thousand, Turkish Community Festival at Russell Square every August, Iraqi Community Festival at the Multicultural Services Centre in Mirrabooka every October. Lebanese food on William Street Northbridge and Bannister Road Canning Vale, Persian food in Willetton, Turkish food in Cannington. The Lebanese Australian Community Society of WA and Iranian Community Society of WA coordinate year-round events. EventLinqs is built for Perth Middle Eastern organisers running events that anchor a Levantine community on Australia's western edge.",
  },
  ...
  'gospel/gold-coast': {
    ...
    editorial:
      "The Gold Coast Gospel scene is mid-sized and lifestyle-led. Pan-African Gospel services across Robina pull eight hundred on Sundays, Pacific Islander Methodist Gospel choirs in Helensvale rehearse for outdoor events, Caribbean Gospel services across Mt Tamborine blend reggae and worship, Filipino Tagalog Mass at Clear Island Waters fills Sacred Heart, Korean Hillsong worship at Hillsong Gold Coast pulls one thousand. Gold Coast Gospel Festival every August at HOTA pulls three thousand. Gospel choir competitions through the wider region every May. The Gold Coast Pentecostal Choir Network coordinates the year-round calendar. The Beach Gospel Sunday at Burleigh Beach through summer pulls a diverse worship crowd. EventLinqs is built for Gold Coast Gospel organisers running events that match coast lifestyle to Sunday song.",
  },
  'gospel/canberra': {
...
function fallbackEditorial(community: CommunityContent, cityName: string, cityRecord: CityContent | null): string {
  ...
  /*
   * DO NOT LOWERCASE displayName HERE. Every one of the 21 values is a proper
   * adjective (African, Filipino, Greek, Aboriginal & Torres Strait Islander),
   * so lowercasing published "has a aboriginal & torres strait islander
   * community" on a live page. Found on 3 September 2026 by rendering the page
   * rather than reading the template.
   *
   * The sentences are also phrased to avoid "a" before the name, because the
   * article cannot agree with both "a Greek community" and "an African
   * community" from one template.
   */
  const stateLine = cityRecord?.state
    ? `The ${community.displayName} community in ${cityName}, ${cityRecord.state}, books every kind of venue, from community halls to live-music venues to outdoor festival sites.`
    : `The ${community.displayName} community in ${cityName} books every kind of venue.`
  return `${stateLine} ${subCommunitySentence} EventLinqs is built for ${community.displayName} organisers in ${cityName} who want transparent fees, WhatsApp-first sharing, and squad bookings so the whole crew comes together. ${community.tagline}`
}

 ...
 * lives in src/app/api/webhooks/stripe/route.ts, which is owned by the backend
 * session. The hook-up is handed to that session (see
 * docs/launch-hardening/payouts-dashboard-landing.md); this lib ships ready and
 * unit-tested so the wiring is a small, isolated change there.
 */
...
function shell(heading: string, body: string, dashboardUrl: string): string {
  return `
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;">
      <h1 style="color:#0A1628;font-size:22px;margin-bottom:8px;">${heading}</h1>
      <hr style="border:none;border-top:1px solid #e5e7eb;margin:16px 0;" />
      <div style="color:#374151;font-size:14px;line-height:1.6;">${body}</div>
      <p style="margin-top:24px;">
        <a href="${dashboardUrl}"
           style="display:inline-block;background:#0A1628;color:white;padding:12px 20px;border-radius:8px;text-decoration:none;font-weight:600;">

...
/**
 * Sub-culture photo pipeline backed by Pexels.
 *
 * Mirrors culture-photo.ts but for the sub-cultures rail tiles. Tier 1
 * cultures expose 4-6 sub-cultures each (filipino 4; mediterranean and
 * middle-eastern 5; the rest 6). Each gets a culture-specific Pexels
 ...
 * fails. The SubCultureTileImage component substitutes a branded
 * navy gradient when null.
 *
 * Key shape: `${cultureSlug}:${subCultureSlug}` so sub-cultures with
 * the same slug across different cultures (none today, but cheap
...
const SUB_CULTURE_QUERIES: Record<string, string> = {
  // ---- African (6) -------------------------------------------------
...
async function fetchSubCulturePhotoRaw(query: string): Promise<string | null> {
  ...
  try {
    ...
    const photo = pool[simpleHash(query + ':sub-culture') % pool.length]
...
const fetchSubCulturePhoto = unstable_cache(
  fetchSubCulturePhotoRaw,
  ['pexels-sub-culture-v1'],
  { revalidate: 60 * 60 * 24 * 7, tags: ['pexels', 'pexels-sub-culture'] }
)

export async function getSubCulturePhoto(
  cultureSlug: string,
  subCultureSlug: string,
): Promise<string | null> {
  const key = `${cultureSlug.toLowerCase()}:${subCultureSlug.toLowerCase()}`
  const query = SUB_CULTURE_QUERIES[key]
  if (!query) return null
  return await fetchSubCulturePhoto(query)
}

export function isKnownSubCultureKey(cultureSlug: string, subCultureSlug: string): boolean {
  return `${cultureSlug.toLowerCase()}:${subCultureSlug.toLowerCase()}` in SUB_CULTURE_QUERIES
}
